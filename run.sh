...
translation_path="${base_str}${dataname}.trans"
fairseq-generate $DATA \
    --path $save_dir/checkpoint_last.pt \
    --batch-size 128 --beam 5 --remove-bpe \
    --dataset-impl raw | tee $translation_path

# Compute BLEU score
grep ^H $translation_path | cut -f3- > $translation_path.sys
grep ^T $translation_path | cut -f2- > $translation_path.ref
fairseq-score --sys $translation_path.sys --ref $translation_path.ref
